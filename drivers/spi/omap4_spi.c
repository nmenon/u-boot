 ...
 *  SPI clock devider
 */
static unsigned long SPI_GetClockDevider(void)
{
	gpMCSPIRegs = (MCSPI_REGS *)MCSPI_SPI1_IO_BASE;
...
static void spi_init_spi1(void)
{
	...
	sr32((u32 *)0x4A100130, 0, 32, 0x01100118);
	sr32((u32 *)0x4A100134, 0, 32, 0x01100110);
	sr32((u32 *)0x4A100138, 0, 32, 0x01130110);

	/* Output enable && dataout 1 for gpio 48 to power up ethernet */
	sr32((u32 *)0x48055134, 16, 1, 0);
	sr32((u32 *)0x4805513C, 16, 1, 1);

	/*
	 * Output enable && dataout 1 for gpio 138
	 * as a workaround to keep CS1 high
	 */
	sr32((u32 *)0x4805B134, 10, 1, 0);
	sr32((u32 *)0x4805B13C, 10, 1, 1);
	sr32((u32 *)0x48098110, 0, 32, 0x00000308);
...
unsigned short spi_ks8851snl_reg_read(unsigned short read_cmd)
{
	...
	/* Give some time for the controller to react */
	udelay(1);
